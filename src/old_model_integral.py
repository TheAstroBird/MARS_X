...
n = 2
DATAs = pd.read_excel("../data/dynamic/model_distributions.xlsx")
n_grid = DATAs.shape[0]
rad = np.zeros(n_grid)
den = np.zeros(n_grid)
v_p = np.zeros(n_grid)
v_s = np.zeros(n_grid)
...
for i in range(n_grid):
    rad[i] = DATAs['radius'][n_grid - 1 - i]
    den[i] = DATAs['density'][n_grid - 1 - i]
    v_p[i] = DATAs['compr velocity'][n_grid - 1 - i]
    v_s[i] = DATAs['shear velocity'][n_grid - 1 - i]
    grav[i] = DATAs['gravity'][n_grid - 1 - i]

DATAadd = pd.read_excel("../data/dynamic/input_param.xlsx")
sulf = DATAadd['core sulfur']
hydr = DATAadd['core hydro']
...
transit4 = n_grid - 1 - 4205
transit3 = n_grid - 1 - 3892
transit2 = n_grid - 1 - 3266
transit1 = n_grid - 1 - 3088
i_core = n_grid - 1 - 5157
i_crust = n_grid - 1 - 150
den_crust = den[n_grid-1]
den_core = den[0]
if MELTLAYER:
    di_melt = n_grid - 1 - 4566 - i_core
else:
    di_melt = 0
...
eta_melt = math.log10(DATAadd['visc_melt'][0]) - 9
